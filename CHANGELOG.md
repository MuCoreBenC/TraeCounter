...
- **移除校准锁定功能**：count 偏高是正常现象（消息太长被截断 → 点击继续 → Trae 创建新 git tag，但服务端 used 不增加）。完全移除校准锁定逻辑，`displayCount = count` 始终，让计数器记录和继续显示真实值

### 优化

- **额度检测扩展数据来源（方案 C）**：`parseQuotaFromLogReverse` 从仅匹配 `code: 4031` 错误行，扩展为匹配任何包含 `extra: Some("...")` 且 extra JSON 含 `quota`/`used` 字段的日志行。新增 `extra_log` 来源标识与 `4031_log` 区分，4031 优先级仍高于其他来源（更可靠）。当 Trae 升级变更错误码时仍能工作，且能捕获非耗尽状态下的额度信息
- **账号切换清空 quota 缓存**：`SetOnCurrentUserChanged` 回调中调用 `InvalidateQuotaCache()`，修复已有隐患（之前该函数定义了但从未被调用，切换账号后 10 分钟内仍显示上一个账号的额度信息）
- **新增指定版本构建脚本**：`构建指定版本.command` 双击运行，提示输入版本号（自动建议 patch+1），调用 `build.sh` 构建

## v0.2.14

### 优化

- **README 简化**：公开仓库文案改为简略大众版，不涉及技术细节；私有仓库预览图片改为 2x2 网格布局
