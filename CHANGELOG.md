...
- **额度校准开关**：到达上限后自动用服务端 4031 返回的精确数值校准计数，并锁定不再增加，直到 `next_flash` 重置时间过期。设置面板新增"额度校准"开关，大数字右上角显示蓝色"已校准"标识
- **本周视图 X 轴标签点击切换周几**：本周视图下点击 X 轴日期标签区域，可在"日期（12-01）"和"周几（周一）"之间切换显示
- **长按防抖按钮**：设置面板所有数值调节的 +/- 按钮支持长按持续递增/递减（按下立即触发一次，400ms 后以 80ms 间隔重复）

### 修复

- **额度检测完全失效**：3 个致命 bug 导致 4031 错误检测全部失败
  - 正则 `error_code:4031` 应为 `code: 4031`（实际 Trae 日志格式）
  - `extra` 字段格式 `extra:"{...}"` 应为 `extra: Some("{...}")`（Rust Debug 输出）
  - `hasFastRequestDisabled` 日期判断逻辑反转，导致永远返回 false
- **深色模式滚动条 hover 变白**：深色模式下滚动条悬停时颜色变灰白，添加自定义 scrollbar 样式修复

### 优化

- **额度检测算法重构**：采用反向分块扫描（从文件末尾向前读取 4MB 块），常规模式 1 块（4MB），深度模式 5 块（20MB）。当今日计数达到已知额度 80% 时自动触发深度扫描，避免日常高开销
- **双维度额度信息**：支持 daily（日额度）和 weekly（周额度）双维度解析，Free 用户日额度 43、周额度 120
- **智能阈值增强**：开启智能阈值后，额度上限显示学习到的值（只读），提醒/警告阈值根据学习上限自动计算（学习上限 - 10 / - 5），灰色显示不可修改；首次未学习到时用硬编码 43 作为基准
- **设置项 Tooltip 化**：所有设置项副标题改为 info 图标悬停显示，节省垂直空间
- **步进改为 1**：额度上限、提醒阈值、警告阈值的步进从 5 改为 1，约束放宽为仅要求提醒 ≤ 警告（允许相等）
- **用户管理优先显示备注名**：右上角用户管理区域优先显示备注名

## v0.2.13

### 修复

- **应用"睡死"不自动刷新**：原架构完全依赖 fsnotify 事件驱动，无后备机制。macOS 睡眠/唤醒后 fsnotify 可能失效，watcher goroutine 崩溃后无人重启，导致应用"睡死"不自动刷新数据。新增每 30 分钟定时全量 `Refresh()` 作为后备机制，保证即使 fsnotify 失效也能更新数据。如果 fsnotify 不可用，降级为纯轮询模式
